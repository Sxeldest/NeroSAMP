...
void ProgressBar::draw(ImGuiRenderer* renderer)
{
	...
	// scissor test
	renderer->pushClipRect(absolutePosition(),
			absolutePosition() + ImVec2(width() * value(), height()));
	...
	switch (m_type) {
		...
		case ProgressBarType::MULTICOLOR:
			renderer->drawRectFilledMulticolor(absolutePosition(), absolutePosition() + size(),
					m_colFilledUprLeft, m_colFilledUprRight, m_colFilledBotRight,
					m_colFilledBotLeft);
			break;
	}
	...
	if (m_border) {
		renderer->drawRect(absolutePosition(), absolutePosition() + size(),
				ImColor(0.0f, 0.0f, 0.0f), false, UISettings::outlineSize());
	}
}