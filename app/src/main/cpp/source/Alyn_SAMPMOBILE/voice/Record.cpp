...
bool Record::Init(const uint32_t bitrate) noexcept
{
	...
	memory::ScopeExit deviceListsResetScope{[] {
...
void Record::Tick() noexcept
{
	if (Record::initStatus && Record::checkStatus) {
		if (const auto bufferSize = BASS_ChannelGetData(Record::recordChannel,
			...
			if (const auto readDataSize = BASS_ChannelGetData(Record::recordChannel,
						Record::encBuffer.data(),
...
uint32_t Record::GetFrame(uint8_t* const bufferPtr, const uint32_t bufferSize) noexcept
{
	...
	const auto cBufferSize = BASS_ChannelGetData(Record::recordChannel, nullptr,
			BASS_DATA_AVAILABLE);
...
void Record::StopRecording() noexcept
{
	...
	const auto bufferSize = BASS_ChannelGetData(Record::recordChannel, nullptr,
			BASS_DATA_AVAILABLE);
