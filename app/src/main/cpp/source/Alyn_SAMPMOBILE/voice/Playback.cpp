...
{
	if (!Playback::loadStatus) {
		return;
	}

	MATRIX4X4* matPos = (MATRIX4X4*) (SA_Addr(0x9528A4));

	BASS_Set3DPosition(
			reinterpret_cast<const BASS_3DVECTOR*>(&matPos->pos), nullptr,
			reinterpret_cast<const BASS_3DVECTOR*>(&matPos->at),
			reinterpret_cast<const BASS_3DVECTOR*>(&matPos->up));
...
void Playback::SetSoundBalancer(const bool soundBalancer) noexcept
{
	...
	if (PluginConfig::GetSoundBalancer() && balancerFxHandle == NULL) {
		balancerFxHandle = BASS_ChannelSetFX(Playback::deviceOutputChannel,
				BASS_FX_BFX_COMPRESSOR2);

		if (balancerFxHandle == NULL) {
			LogVoice("[sv:err:playback:setsoundbalancer] : failed to set balancer effect (code:%d)",
					BASS_ErrorGetCode());
		...
		if (BASS_FXSetParameters(balancerFxHandle, &balancerParameters) == 0) {
			LogVoice("[sv:err:playback:setsoundbalancer] : failed to set parameters (code:%d)",
					BASS_ErrorGetCode());
...
void Playback::SetSoundFilter(const bool soundFilter) noexcept
{
	...
	if (PluginConfig::GetSoundFilter() && filterFxHandle == NULL) {
		...
		if (filterFxHandle == NULL) {
			LogVoice("[sv:err:playback:setsoundfilter] : failed to set filter effect (code:%d)",
					BASS_ErrorGetCode());
		...
		if (BASS_FXSetParameters(filterFxHandle, &parameqParameters) == 0) {
			LogVoice("[sv:err:playback:setsoundfilter] : failed to set parameters (code:%d)",
					BASS_ErrorGetCode());
...
bool Playback::BassInitHookFunc() noexcept
{
	...
	LogVoice("[sv:dbg:playback:bassinithook] : calling function BASS_Init(device:-1, "
			 "freq:%u, flags:0x%x)...",
			SV::kFrequency, BASS_DEVICE_MONO | BASS_DEVICE_3D);

	if (BASS_Init(-1, SV::kFrequency, BASS_DEVICE_MONO | BASS_DEVICE_3D) == 0) {
		LogVoice("[sv:err:playback:bassinithook] : failed to init bass library (code:%d)",
				BASS_ErrorGetCode());
		//return false;
	}
	...
	if (Playback::deviceOutputChannel == NULL) {
		LogVoice("[sv:err:playback:init] : failed to create device output channel (code:%d)",
				BASS_ErrorGetCode());
		return false;
	}
