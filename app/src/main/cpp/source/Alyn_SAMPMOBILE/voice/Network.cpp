...
bool Network::Init() noexcept
{
	...
	if (const int sendBufferSize{kSendBufferSize}, recvBufferSize{kRecvBufferSize};
			setsockopt(Network::socketHandle, SOL_SOCKET, SO_SNDBUF,
					reinterpret_cast<const void*>(&sendBufferSize), sizeof(sendBufferSize))
					== SOCKET_ERROR ||
					setsockopt(Network::socketHandle, SOL_SOCKET, SO_RCVBUF,
							reinterpret_cast<const void*>(&recvBufferSize), sizeof(recvBufferSize))
							== SOCKET_ERROR) {
		return false;
	}
...
bool Network::SendControlPacket(const uint16_t packet, const void* dataAddr,
		const uint16_t dataSize) noexcept
{
	...
	return SAMP::netgame()->m_rakClient->Send(&bsSend, HIGH_PRIORITY, RELIABLE, 0);
}
...
void Network::VoiceThread() noexcept
{
	...
	while (true) {
		...
		if (curStatus == ConnectionStatus::Disconnected) {
			break;
		}
		...
		if (!Network::inputVoicePacket->CheckHeader()) {
			continue;
		}
		if (received != Network::inputVoicePacket->GetFullSize()) {
			continue;
		}
		if (Network::inputVoicePacket->packet == SV::VoicePacketType::keepAlive) {
			continue;
		}
...
bool Network::OnRaknetRpc(const int id, RakNet::BitStream& parameters) noexcept
{
	...
	if (!once) {
		LogVoice("[sv:dbg:network:rpc] : rpc received.");
		once = true;
	}

	if (!Network::initStatus || id != kRaknetConnectRcpId) {
		LogVoice("[sv:err:network:rpc] : not initStatus || invalid rpc id (%i).", id);
		return true;
	}

	if (Network::svConnectCallbacks.empty()) {
		LogVoice("[sv:err:network:rpc] : no svConnectCallbacks.");
		return true;
	}
...
bool Network::OnRaknetReceive(int packet) noexcept
{
	static bool once = false;
	if (!once) {
		LogVoice("[sv:dbg:network:receive] : packet received.");
		once = true;
	}

	if (!Network::initStatus) {
		LogVoice("[sv:err:network:receive] : network not initialized.");
		return true;
	}

	uint32_t packetLength = *(uint32_t*) (packet + 12);
	uint8_t* packetData = *(uint8_t**) (packet + 20);

	if (packetLength < sizeof(uint8_t) + sizeof(ControlPacket)) {
		LogVoice("[sv:err:network:receive] : invalid packet length (%u).", packetLength);
		return true;
	}

	if (*packetData != kRaknetPacketId) {
		LogVoice("[sv:err:network:receive] : invalid packet id (%hhu).", *packetData);
		return true;
	}

	const auto controlPacketPtr = reinterpret_cast<ControlPacket*>(packetData + sizeof(uint8_t));
	const uint32_t controlPacketSize = packetLength - sizeof(uint8_t);

	if (controlPacketSize != controlPacketPtr->GetFullSize()) {
		LogVoice("[sv:err:network:receive] : invalid packet size (%u).", controlPacketSize);
		return false;
	}
	...
	switch (controlPacketPtr->packet) {
		case SV::ControlPacketType::serverInfo: {
			...
			if (controlPacketPtr->length != sizeof(stData)) {
				return false;
			}
		...
		case SV::ControlPacketType::pluginInit: {
			...
			break;
		default: {
			Network::controlQueue.try_emplace(
					MakeControlPacketContainer(controlPacketPtr, controlPacketSize));
		}
	}
